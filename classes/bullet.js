        ...
        const moveBullet = () => {
            ...
            for (let invader of allInvaders) {
                ...
                if (isCollidingWithInvader) {
                    ...
                    let x = parseInt(invader.getAttribute("data-x"))
                    let y = parseInt(invader.getAttribute("data-y"))

                    const index = invaders.findIndex((obj) => obj.x === x && obj.y === y);
                    if (index !== -1) {
                        invaders.splice(index, 1); // ✅ Mutates the array without reassigning
                    }
